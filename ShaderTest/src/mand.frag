...
void main() //|||||||||||||||||||||||||||||MAIN|||||||||||||||||||||||||||||||||||||||||||||||||||||||||
{
    ...
    case 36: v = mand(windowCoordD(), dvec2(0.0), vec2(abs(pc.sinTime)),    2.0,false); break;
    case 37: v = mand(windowCoordD(), dvec2(0.0), vec2(abs(pc.sinTime), 0), 2.0,false); break;
    case 38: v = mand(windowCoordD(), dvec2(0.0), vec2(abs(pc.sinTime), 1), 2.0,false); break;
    case 39: v = mand(windowCoordD(), dvec2(0.0), vec2(0, abs(pc.sinTime)), 2.0,false); break;
    case 40: v = mand(windowCoordD(), dvec2(0.0), vec2(1, abs(pc.sinTime)), 2.0,false); break;
    case 41: v = mand(windowCoordD(), dvec2(0.0), vec2(abs(pc.sinTime)),    2.0, true); break;
    case 42: v = mand(windowCoordD(), dvec2(0.0), vec2(abs(pc.sinTime), 0), 2.0, true); break;
    case 43: v = mand(windowCoordD(), dvec2(0.0), vec2(abs(pc.sinTime), 1), 2.0, true); break;
    case 44: v = mand(windowCoordD(), dvec2(0.0), vec2(0, abs(pc.sinTime)), 2.0, true); break;
    case 45: v = mand(windowCoordD(), dvec2(0.0), vec2(1, abs(pc.sinTime)), 2.0, true); break;
